...
import { fetchFeesData } from '../Slice/slice';


function FeeStructure() {
  const { courseData,paidFees } = useSelector(state => state.rootReducer);
  ...
  useEffect(() => {
    dispatch(fetchFeesData({ id: std_id }));
  }, [dispatch, std_id]);
  useEffect(() => {
    // Calculate total price when course details change
    let totalPrice = 0;
    courseData.forEach(course => {
      totalPrice += parseInt(course.price, 10);
    });
    setTotal(totalPrice);
  }, [courseData]);
  return (
              ...
              {courseData.map((course, index) => (
                      <tr key={index}>
                        ...
                        <td className="border-bottom-0">
                          <h6 className="fw-semibold mb-1">{course.course_name}</h6>
                        </td>
                        <td className="border-bottom-0">
                          <h6 className="fw-semibold mb-1">{course.duration}</h6>
                        </td>
                        <td className="border-bottom-0">
                          <h6 className="fw-semibold mb-1">{course.price}</h6>
                        </td>
                      </tr>
                    ))}
                                                <tr>
                    <td colspan="3"><h6 className="fw-semibold mb-0 text-center">Total Fees</h6></td>
                    <td><h6 className="fw-semibold mb-0 text-center">{total}</h6></td>
                    </tr>
                    <tr className="bg-success">
  <td colspan="3"><h6 className="fw-semibold mb-0 text-center">You Paid Fees</h6></td>
  <td><h6 className="fw-semibold mb-0 text-center">{paidFees.paid_fees}</h6></td>
</tr>
<tr className="bg-danger">
  <td colspan="3"><h6 className="fw-semibold mb-0 text-center">Remaining Fees</h6></td>
  <td><h6 className="fw-semibold mb-0 text-center">{total - paidFees.paid_fees}</h6></td>
</tr>
              </tbody>
                          </table>
          </div>
